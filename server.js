...
// Arena runtime bootstrap

const __arenaRuntimePromise = import('./server/modules/arena/index.js')
  .then((arenaModule) => {
    if (typeof defaultProgress === 'function') {
      const originalDefaultProgress = defaultProgress;
      defaultProgress = function wrappedDefaultProgress() {
        return arenaModule.ensureArenaProgress(originalDefaultProgress());
      };
    }
    if (typeof sanitizeProgress === 'function') {
      const originalSanitizeProgress = sanitizeProgress;
      sanitizeProgress = function wrappedSanitizeProgress(progress) {
        return arenaModule.ensureArenaProgress(originalSanitizeProgress(progress));
      };
    }
    if (typeof progressSnapshot === 'function') {
      const originalProgressSnapshot = progressSnapshot;
      progressSnapshot = function wrappedProgressSnapshot(progress) {
        return arenaModule.ensureArenaProgress(originalProgressSnapshot(progress));
      };
    }
    if (typeof effectiveRoomName === 'function') {
      const originalEffectiveRoomName = effectiveRoomName;
      effectiveRoomName = function wrappedEffectiveRoomName(player) {
        if (player && typeof player.currentRoomId === 'string' && player.currentRoomId.startsWith('arena:')) {
          return player.currentRoomId;
        }
        return originalEffectiveRoomName(player);
      };
    }
  ...
  .catch((error) => {
    console.error('Arena runtime failed to initialize', error);
    return null;
  });
